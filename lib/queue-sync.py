...
Uses AVS environment variables (set via paths.sh or directly):
  - AVS_WORKSPACE: Path to workspace root
  - AVS_ARTIFACT_DIR: Path to completion artifacts
  - AVS_QUEUE_FILE: Path to queue YAML file
  - AVS_BLOCKERS_FILE: Path to blockers log
"""
...
# Get paths from environment with sensible defaults
AVS_WORKSPACE = os.environ.get("AVS_WORKSPACE", "/Users/serenerenze/bob-bootstrap")
AVS_ARTIFACT_DIR = os.environ.get(
    "AVS_ARTIFACT_DIR", f"{AVS_WORKSPACE}/OPERATIONAL/completion-artifacts"
)
QUEUE_FILE = os.environ.get("AVS_QUEUE_FILE", f"{AVS_WORKSPACE}/autonomous-queue.yaml")
BLOCKED_LOG = os.environ.get("AVS_BLOCKERS_FILE", f"{AVS_WORKSPACE}/BLOCKERS.md")
...
def find_artifacts_for_task(task_id):
    ...
    pattern = f"{AVS_ARTIFACT_DIR}/TASK_{task_id}_*.yaml"
...
def sync_task_status(task):
    ...
    latest_artifact = max(
        artifacts,
        key=lambda p: parse_timestamp(p.split("_")[-1].replace(".yaml", ""))
        or datetime.min,
    )
    ...
    if artifact_status == "failed" and task["status"] != "failed":
        ...
        return (
            True,
            f"Task {task_id}: MARKED FAILED (artifact: {artifact.get('failure_mode')})",
        )
...
def main():
    ...
    os.makedirs(AVS_ARTIFACT_DIR, exist_ok=True)
